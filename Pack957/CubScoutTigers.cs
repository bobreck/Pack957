...
namespace Pack957
{
	public partial class CubScoutTigers : DialogViewController
	{
		...
		public CubScoutTigers () : base (UITableViewStyle.Grouped, null)
		{
		...
		private void ReloadCubScouts(Section mySection)
		{
			...
			var query = conn.Table<CubScout>().Where(v => v.ScoutType.StartsWith ("0")).OrderBy(x => x.LastName);
